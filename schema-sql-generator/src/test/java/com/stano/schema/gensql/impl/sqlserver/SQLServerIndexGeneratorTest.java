...
@DisplayName("SQLServerIndexGenerator")
class SQLServerIndexGeneratorTest {

  @Test
  @DisplayName("renders a unique filtered index with INCLUDE, WHERE, then WITH in order")
  void rendersUniqueFilteredIndexWithIncludeWhereWith() {
    ...
    assertTrue(
        sql.contains(
            "create unique index ix_parenttable1 on dbo.ParentTable (ParentId) "
  ...
  private String generateIndexesSql(Table table) {
    ...
    IndexGenerator gen = new SQLServerIndexGenerator(sqlGen);
